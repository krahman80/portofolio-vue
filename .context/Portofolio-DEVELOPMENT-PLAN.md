...
- [ ] Port design tokens from `index.html`'s `@theme` block into Tailwind config / CSS variables (colors, font stacks)
...
- [ ] Deploy the empty shell to the target static host (Netlify/Vercel/GitHub Pages — pick one) to prove the pipeline end-to-end
...
- [ ] Deploy to the chosen static host with a proper domain/subdomain
